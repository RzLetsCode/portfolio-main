...
export default function Home() {
  const skills = [
    { 
      category: 'AI & ML', 
      items: ['LLMs', 'RAG Pipelines', 'Multi-Agent Systems', 'Azure OpenAI', 'Pinecone', 'LangChain', 'LlamaIndex', 'AutoGPT', 'Vector Databases', 'Prompt Engineering'] 
    },
    { 
      category: 'Data Platforms', 
      items: ['Snowflake', 'Databricks', 'Azure Data Factory', 'Azure Synapse', 'Python', 'SQL', 'ETL', 'Data Lakehouse', 'Spark', 'Delta Lake'] 
    },
    { 
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500/30">
      ...
      {/* Project Detail Anchor Sections (Editable placeholders as requested) */}
      {projects.map((project) => (
        <section key={project.id} id={`project-${project.id}`} className="py-32 px-6 bg-slate-900/20 border-t border-slate-800/50">
          <div className="max-w-4xl mx-auto">
            <Link href="#projects" className="text-cyan-400 flex items-center gap-2 mb-12 font-bold hover:underline">
              ← Back to Projects
            </Link>
            <h2 className="text-4xl md:text-5xl font-black mb-8">{project.title}</h2>
            
            <div className="prose prose-invert max-w-none">
              <div className="grid md:grid-cols-3 gap-8 mb-16">
                <div className="p-6 bg-slate-800/40 rounded-3xl border border-slate-700/50">
                  <h4 className="text-cyan-400 font-black uppercase tracking-widest text-xs mb-4">Architecture</h4>
                  <p className="text-sm text-slate-300 leading-relaxed font-medium">Distributed pipeline with vector caching and semantic re-ranking for optimal performance.</p>
                </div>
                <div className="p-6 bg-slate-800/40 rounded-3xl border border-slate-700/50">
                  <h4 className="text-cyan-400 font-black uppercase tracking-widest text-xs mb-4">Scalability</h4>
                  <p className="text-sm text-slate-300 leading-relaxed font-medium">Deployed using containerized microservices on Azure Kubernetes Service (AKS).</p>
                </div>
                <div className="p-6 bg-slate-800/40 rounded-3xl border border-slate-700/50">
                  <h4 className="text-cyan-400 font-black uppercase tracking-widest text-xs mb-4">Tech Stack</h4>
                  <p className="text-sm text-slate-300 leading-relaxed font-medium">{project.tech.join(', ')}</p>
                </div>
              </div>

              <h3 className="text-2xl font-bold mb-6 text-slate-100">Project Overview</h3>
              <p className="text-slate-400 text-lg leading-relaxed mb-10 font-medium">
                {project.description} This implementation involved designing a custom orchestration layer that handles complex queries by routing them to specialized {project.id === 'multi-agent' ? 'agents' : 'knowledge stores'}. 
                The primary challenge was ensuring low latency while maintaining high accuracy in {project.id === 'healthcare-analytics' ? 'predictive insights' : 'contextual retrieval'}.
              </p>

              <h3 className="text-2xl font-bold mb-6 text-slate-100">Key Features & Implementation</h3>
              <ul className="space-y-4 mb-12">
                {['Custom optimization of LLM context windows', 'Implementation of robust error handling and fallback mechanisms', 'Integrated real-time monitoring and logging', 'Automated CI/CD pipelines for model updates'].map((item, idx) => (
                  <li key={idx} className="flex items-start gap-4 text-slate-300 font-medium">
                    <span className="text-cyan-400 mt-1.5"><Zap size={14} /></span>
                    {item}
                  </li>
                ))}
              </ul>
              ...
              <div className="p-8 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 rounded-3xl border border-cyan-500/20">
                <h3 className="text-xl font-black mb-4 text-cyan-400">Business Impact</h3>
                <p className="text-slate-200 font-semibold text-lg italic underline decoration-cyan-500/30">
                  {project.details}
                </p>
              </div>
            </div>
      ...
      <section id="contact" className="py-32 px-6">
        <div className="max-w-5xl mx-auto bg-slate-900 rounded-[3rem] border border-slate-800 p-16 text-center relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent"></div>
          <div className="relative z-10">
            <h2 className="text-5xl font-black mb-8 leading-tight">Ready to build the <span className="text-cyan-400 underline decoration-cyan-500/20">Next Generation</span> of AI?</h2>
            <p className="text-xl text-slate-400 mb-16 max-w-2xl mx-auto font-medium leading-relaxed">Let's discuss Enterprise AI Architecture, Databricks pipelines, or Azure Cloud scaling.</p>
            
            <div className="flex flex-wrap justify-center gap-8">
              <Link href="mailto:rajeshrajrz111@gmail.com" className="flex items-center gap-4 px-10 py-5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 rounded-2xl transition-all shadow-xl shadow-cyan-500/10 font-black active:scale-95">
                <Mail />
                <span>Email Me</span>
              </Link>
              <Link href="https://linkedin.com/in/rajesh-kumar-04405962/" target="_blank" className="flex items-center gap-4 px-10 py-5 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl transition-all border border-slate-700 font-black active:scale-95">
                <Linkedin />
                <span>Connect LinkedIn</span>
              </Link>
              <Link href="https://www.youtube.com/@Code2Career_AI" target="_blank" className="flex items-center gap-4 px-10 py-5 bg-red-600 hover:bg-red-500 text-white rounded-2xl transition-all shadow-xl shadow-red-600/10 font-black active:scale-95">
                <Youtube />
                <span>YouTube</span>
              </Link>
            </div>
            
            <div className="mt-20 pt-16 border-t border-slate-800/50 flex flex-col md:flex-row justify-between items-center gap-8 text-slate-500 font-bold tracking-wider text-xs">
              <p>© 2026 RAJESH KUMAR • ENTERPRISE AI ARCHITECT</p>
              <div className="flex gap-8">
                <Link href="https://hashnode.com/@rz111" target="_blank" className="hover:text-cyan-400 transition-colors">HASHNODE</Link>
